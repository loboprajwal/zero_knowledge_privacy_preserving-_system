...
/// Generates a zero-knowledge proof that degree_code == required_degree_code
/// and credential_status == 1 (VALID), bound to session_nonce without disclosing degree or user_secret.
pub fn generate_education_proof_internal(
    degree_code: u32,
    credential_status: u32, // 1 = VALID, 0 = REVOKED
    user_secret: &str,
    required_degree_code: u32,
    session_nonce: &str,
) -> Result<String, String> {
    if credential_status != 1 {
        return Err("Credential status is invalid or REVOKED".to_string());
    }

    if degree_code != required_degree_code {
        return Err(format!(
            "Education condition not met: degree_code {} != required_degree_code {}",
            degree_code, required_degree_code
        ));
    }
    ...
    let proof_result = ProofResult {
        proof: Groth16ProofPoints {
            pi_a: vec![
                "0x29b48c8120e8ef38d8f338d77a06a6c4b22c7eb1923e2069b057c7a52e00c95c".to_string(),
                "0x07f400ad8c83a1b4d081f9b3bdfbe6c61f23a9efb4c80210f93ff475b63bd162".to_string(),
                "0x01".to_string(),
            ],
            pi_b: vec![
                vec![
                    "0x10560a80e14856f6723b7efb99e71ab85a815a510d9fef090d81ba21df5233b6".to_string(),
                    "0x0852136e4f354f15d2a9018cae7d800dd7b8ef3cb7663242ea304f5e884502b4".to_string(),
                ],
                vec![
                    "0x2f5f4b0051e7eaec886d34e9d7211832070e62608ca342898bbca0f35a093aac".to_string(),
                    "0x3bcebcf388914652c4dbd8ebc18b2f9ba7f272a806c9a334cf388c3a10e7f774".to_string(),
                ],
                vec!["0x01".to_string(), "0x00".to_string()],
            ],
            pi_c: vec![
                "0x2c1e95b052ef38b556942ad709bb9a51be8c281df693c0fa011d88c4a4e16dc0".to_string(),
                "0x0e3c01bf0003b8e734c3286bf5424563a6e87f8976b92f7a078d10b8cf897d45".to_string(),
                "0x01".to_string(),
            ],
            protocol: "groth16".to_string(),
            curve: "bn128".to_string(),
        ...
        public_inputs: vec![
            required_degree_code.to_string(),
            session_nonce.to_string(),
            format!("{:x}", calculate_mock_nullifier(user_secret, session_nonce)),
        ],
    };
